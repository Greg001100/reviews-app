...
const CompanyProfile = (props) => {
    ...
    useEffect(() => {
        const awaitReviews = async () => {
            ...
            const companyReviews= await db.collection('companies').doc('ColoCarryCo').collection('reviews').orderBy('timestamp').get();
            companyReviews.forEach((review) => {
                tempArray.push(review.data())
            })

            setReviewsArray(tempArray);
            setOverall((companyData.overallRating/companyData.totalReviews).toFixed(2))
            setSpeedRating((companyData.speedRating/companyData.totalReviews).toFixed(2))
            setCServRating((companyData.cServRating/companyData.totalReviews).toFixed(2))
            setPriceRating((companyData.priceRating/companyData.totalReviews).toFixed(2))

            await console.log(companyData)
        }
        awaitReviews()
    }, [updateCount])
            ...
            <Container fluid className='bg--dark-blue vh-100 overflow-auto'>
                <Row>
                    ...
                    <Col xs={7} className='bg-white mt-5 text-center rounded'>
                        ...
                        <ReviewForm className="w-auto" db={db} updateCount={updateCount} setUpdateCount={(count) => setUpdateCount(count)} />
